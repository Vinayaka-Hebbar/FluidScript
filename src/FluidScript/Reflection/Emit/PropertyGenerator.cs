...
{
    public sealed class PropertyGenerator : System.Reflection.PropertyInfo, IMemberGenerator
    {
        ...
        {
            PropertyHolder item = Accessors.FirstOrDefault(acc => acc.PropertyType == Emit.PropertyType.Get && acc.IsPublic == nonPublic);
            if (item != null)
                return item.Method;
        ...
        {
            return Accessors.FirstOrDefault(acc => acc.PropertyType == Emit.PropertyType.Set && acc.IsPublic == nonPublic).Method;
        }
